...
<div id="comments" class="clearfix">
    ...
    <?php if ( post_password_required() ) : ?>
        <p class="nopassword">
            <?php _e( 'This post is password protected. Enter the password to view any comments.', 'mastermind' ); ?>
        </p>
    <?php return; endif;
    $ncom = get_comments_number();
    if ($ncom>0) :
        echo '<h3 class="tbeer-wrapper-title">';
        if ($ncom==1) _e('1 ', 'mastermind'); else echo sprintf (__('%s ','mastermind'), $ncom);
        _e('Comments','mastermind');
        echo '</h3>';
    ...
    <h3 class="tbeer-wrapper-title"><?php _e('Leave a reply','mastermind'); ?></h3>
        ...
        $args = array (
            ...
            'logged_in_as' => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>','mastermind'), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink($post->ID) ) ) ) . '</p>',
        );
